def format_duration(seconds):
    ...
    if len(texts) == 0: return "now"
    if len(texts) == 1: return texts[0]
    if len(texts) == 2: return "{0} and {1}".format(texts[0], texts[1])
    if len(texts) >= 3: return "".join(item + ", " for item in texts[:-2]) + "{0} and {1}".format(texts[-2], texts[-1])
